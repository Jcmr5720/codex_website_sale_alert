...
class MailingList(models.Model):
    ...
    # from O15
    contact_count = fields.Integer(compute='_compute_contact_count', string='# of Contacts')

    def _compute_contact_count(self):
        for rec in self:
            rec.contact_count = len(rec.contact_ids.ids)
    ...
    def _compute_contact_nbr(self):
        ...
        if self.env.context.get('whatsapp_mailing'):
            for rec in self:
                rec.contact_nbr = rec.contact_count
        else:
            return super(MailingList, self)._compute_contact_nbr()
        # if self.env.context.get('whatsapp_mailing') and self.ids:
        #     self.env.cr.execute('''
        ...
        #         where
        #             list_id in %s
        #             AND c.ws_phone_normalized IS NOT NULL
        #             AND c.is_whatsapp = 't'
        #         group by list_id
        #     ''', (tuple(self.ids), ))
